...
$(function() {
    ...
    var events = {
        ...
        publish : function() {
            ...
            var status = hasStatus && $('input[name=status]:checked').val();
            ...
            if ((hasTitle && title.length == 0) 
                ...
                window.wxc.xcConfirm("请补充完整信息！",
                    ...
                    || (hasContent && parseInt($('.count').text()) > Max["content"])) {
                window.wxc.xcConfirm("标题或内容字数超限！",
                        window.wxc.xcConfirm.typeEnum.info);
            } else if ( (onlyImage || onlyFile) && curFiles.length < fileNumMin || curFiles.length > 10) {
                            ...
                            success : function(data) {
                                ...
                                if (data.code == 200) {
                                    window.wxc.xcConfirm("发布成功！",window.wxc.xcConfirm.typeEnum.success, 
                                            {onOk : function(v) {window.location.href = "/backend/histories";}});
                                } else {
    ...
    function event_bind() {
        ...
        $("input[name=principal]").keyup(events.count_char);
        $("input[name=tele]").keyup(events.count_char);
